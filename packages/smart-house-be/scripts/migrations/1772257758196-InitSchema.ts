...
export class InitSchema1772257758196 implements MigrationInterface {
    name = 'InitSchema1772257758196'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`track_financial_transaction\` (\`id\` varchar(36) NOT NULL, \`financial_id\` varchar(255) NOT NULL COMMENT '基金Id', \`user_id\` varchar(255) NOT NULL COMMENT '用户Id', \`transaction_type\` enum ('BUY', 'SELL') NOT NULL COMMENT '交易类型：BUY-买入，SELL-卖出', \`amount\` decimal(10,2) NOT NULL COMMENT '交易金额', \`shares\` decimal(13,6) NOT NULL COMMENT '交易份额，确认份额后会更新，年化的类型没有份额' DEFAULT '0.000000', \`ensure_date\` date NOT NULL COMMENT '确认份额的日期', \`transaction_date\` datetime NOT NULL COMMENT '交易时间', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="基金交易记录表"`);
        await queryRunner.query(`CREATE TABLE \`track_financial\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(32) NOT NULL COMMENT '理财的名称', \`code\` varchar(32) NOT NULL COMMENT '理财编码', \`channel\` enum ('icbc_financial', 'icbc_currency', 'cmb_financial', 'cmb_fund') NOT NULL COMMENT '理财的购买渠道', \`delay\` int UNSIGNED NOT NULL COMMENT '理财等待时间T+N' DEFAULT '1', \`user_id\` varchar(255) NOT NULL COMMENT '绑定的用户', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, UNIQUE INDEX \`ux_code_user_id\` (\`code\`, \`user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="正在追踪的理财记录表"`);
        await queryRunner.query(`CREATE TABLE \`financial_value_trend\` (\`id\` varchar(36) NOT NULL, \`financial_id\` varchar(255) NOT NULL COMMENT '理财Id', \`date\` date NOT NULL COMMENT '当前净值日期', \`balance\` decimal(13,6) NOT NULL COMMENT '当前价值（元）', \`shares\` decimal(13,6) NOT NULL COMMENT '当前份额', \`profit\` decimal(13,6) NOT NULL COMMENT '当日盈利', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, UNIQUE INDEX \`ux_financial_id_date\` (\`financial_id\`, \`date\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="理财价值表"`);
        await queryRunner.query(`CREATE TABLE \`financial_net_value_trend\` (\`id\` varchar(36) NOT NULL, \`code\` varchar(32) NOT NULL COMMENT '理财编码', \`date\` date NOT NULL COMMENT '当前净值日期', \`value\` decimal(10,6) NOT NULL COMMENT '单位净值/万份收益（元）', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, UNIQUE INDEX \`ux_code_date\` (\`code\`, \`date\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB COMMENT="理财净值表"`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP INDEX \`ux_code_date\` ON \`financial_net_value_trend\``);
        await queryRunner.query(`DROP TABLE \`financial_net_value_trend\``);
        await queryRunner.query(`DROP INDEX \`ux_financial_id_date\` ON \`financial_value_trend\``);
        await queryRunner.query(`DROP TABLE \`financial_value_trend\``);
        await queryRunner.query(`DROP INDEX \`ux_code_user_id\` ON \`track_financial\``);
